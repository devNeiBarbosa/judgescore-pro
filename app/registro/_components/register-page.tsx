...
import { Mail, Lock, User, Phone, Eye, EyeOff, Calendar, Instagram } from 'lucide-react';
...
const InviteInfo = styled.div<{ $valid: boolean }>`
  ...
  background: ${({ $valid }) => ($valid ? 'rgba(0,230,118,0.12)' : 'rgba(255,61,61,0.12)')};
  color: ${({ $valid }) => ($valid ? '#00E676' : '#FF3D3D')};
  border: 0.5px solid ${({ $valid }) => ($valid ? 'rgba(0,230,118,0.3)' : 'rgba(255,61,61,0.3)')};
`;
...
const RuleItem = styled.li<{ $met: boolean }>`
  ...
  letter-spacing: 0;
  font-weight: 400;
  color: ${({ $met, theme }) => ($met ? '#22C55E' : (theme?.colors?.textMuted ?? '#606060'))};
  display: flex;
  align-items: center;
  gap: 6px;
    ...
    <PageWrapper>
      <FormCard>
        <Card padding="40px">
          ...
          <Title>CRIAR <GoldText>CONTA</GoldText></Title>
          <Subtitle>Cadastro permitido apenas com convite</Subtitle>
          <InviteInfo $valid={Boolean(inviteToken)}>
            {inviteToken ? 'Convite detectado. Complete seu cadastro.' : 'Token de convite não encontrado nesta URL.'}
          </InviteInfo>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            {(formData?.password ?? '').length > 0 && (() => {
              ...
              const noSeqCheck = digits.length <= 1 || !(() => {
              ...
              return (
                <PasswordRules>
                  ...
                  <RuleItem $met={hasDigit && noSeqCheck}>1 número (sem sequência simples)</RuleItem>
                  <RuleItem $met={hasSpecial}>1 caractere especial (@, !, #, $)</RuleItem>
                </PasswordRules>
              );
            })()}
            <Input
